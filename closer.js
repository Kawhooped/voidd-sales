/* Closer portal. Wren is the floor lead. */
(function (w) {
  ...
  function loadPack() {
    ...
    return fetch(src).then(function (r) { return r.json(); }).then(function (j) { pack = j || pack; return pack; }).catch(function () { return pack; });
  }
  function packAnswer(q) {
    ...
    (pack.items || []).forEach(function (it) {
      ...
      let n = 0; words.forEach(function (w) { if (hay.indexOf(w) >= 0) n++; });
  ...
  function localSignup(body) {
    ...
    if (list.some(function (c) { return c.email === email; })) return { ok: false, error: "That email already has an account. Log in." };
  ...
  function signup(body) {
    if (!LOCAL) { const j = localSignup(body); if (j.ok) save(j); return Promise.resolve(j); }
    return post("/api/closer/signup", body).then(function (j) {
      ...
      return j;
    }).catch(function () { const j = localSignup(body); if (j.ok) save(j); return j; });
  }
  function login(body) {
    if (!LOCAL) { const j = localLogin(body); if (j.ok) save(j); return Promise.resolve(j); }
    return post("/api/closer/login", body).then(function (j) {
      if (j && j.ok) save({ name: j.name, phone: j.phone, email: j.email, token: j.token, confirmed: !!j.confirmed });
      return j;
    }).catch(function () { const j = localLogin(body); if (j.ok) save(j); return j; });
  }
  function chat(text) {
    const s = session() || {};
    const body = { name: s.name || "", phone: s.phone || "", email: s.email || "", text: text };
    if (!LOCAL) return loadPack().then(function () { return { ok: true, who: "Wren", text: packAnswer(text) }; });
    ...
    }).catch(function () { return loadPack().then(function () { return { ok: true, who: "Wren", text: packAnswer(text) }; }); });
  }
  function wrenHtml() {
    return '<div class="reach">' +
      '<p class="reach-kicker"><b>Wren</b> — floor lead. Ask her the pack or what to say on the call. Office line stays below.</p>' +
      '<div id="wren-log" class="wren-log"></div>' +
      '<form id="reach-form"><input id="reach-text" autocomplete="off" placeholder="Ask Wren…" /><button type="submit">Send</button></form>' +
      '<p id="reach-status" class="note"></p>' +
      '<p class="office">Office · <a href="tel:' + TEL + '">' + PHONE + '</a> · <a href="sms:' + TEL + '">Text</a></p></div>';
  }
  function bindWren(root) {
    ...
    function add(who, text) {
      if (!log) return;
      const d = document.createElement("div"); d.className = "wmsg"; d.innerHTML = "<span></span><p></p>";
      d.querySelector("span").textContent = who; d.querySelector("p").textContent = text; log.appendChild(d); log.scrollTop = 99999;
    }
    add("Wren", "I'm Wren. You sell. We install. Ask me the price, the pack, or what to say on the call. Don't invent a case study.");
    form.onsubmit = function (e) {
      ...
      add("You", t); root.querySelector("#reach-text").value = ""; st.textContent = "Wren is looking…";
      chat(t).then(function (j) { add((j && j.who) || "Wren", (j && j.text) || "Text the office 786-660-1778."); st.textContent = ""; });
    };
  }
  w.VOIDDCloser = { PHONE: PHONE, TEL: TEL, session: session, save: save, clear: clear, needAuth: needAuth, signup: signup, login: login, chat: chat, wrenHtml: wrenHtml, bindWren: bindWren, loadPack: loadPack, pack: function () { return pack; } };
})(window);
